...
export class UserSettingsPage extends SpacePageBase {
  ...
  private async handleFormSubmit(event: Event): Promise<void> {
    ...
    try {
      ...
      if (emailChangeRequested && hasAdditionalUpdates) {
        NotificationService.getInstance().warning(
          i18next.t("notifications.emailChangeSeparate"),
        );
        return;
      }
      ...
      if (passwordData?.newPassword || passwordData?.confirmPassword) {
        ...
        // Validate ASCII characters only (printable ASCII: 0x20-0x7E)
        const asciiPrintableRegex = /^[\x20-\x7E]+$/;
        if (passwordData.newPassword && !asciiPrintableRegex.test(passwordData.newPassword)) {
          NotificationService.getInstance().error(
            i18next.t("errors.AUTH_PASSWORD_INVALID_CHARACTERS"),
          );
          return;
        }
      }
